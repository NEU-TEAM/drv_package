...
void MakePlan::removeOutliers(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in, pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_out)
{
...
bool MakePlan::getAveragePoint(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in, pcl::PointXYZ &avrPt)
{
...
void MakePlan::smartOffset(pcl::PointXYZ &p_in, float offset_val)
{
  float y_off = offset_val / sqrt(1 + pow(p_in.x / p_in.y, 2)) * p_in.y / fabs(p_in.y);
  float x_off = p_in.x / p_in.y * y_off;
  p_in.x = p_in.x + x_off;
  p_in.y = p_in.y + y_off;
}

void MakePlan::removeNans(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in, pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_out)
{
  std::vector< int >	index;
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered_nan (new pcl::PointCloud<pcl::PointXYZ>);
...
bool MakePlan::process(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in, float a, float b, float c, float d, pcl::PointXYZ &avrPt)
{
  if (!cloud_in->points.size())
  {
    return false;
