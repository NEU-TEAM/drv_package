      ...
      for(int vv = v_start; vv <= v_end; ++vv)
      {
        if((uu == u && vv!=v) || (uu != u && vv==v))
        {
          ...
          if(d != 0.0f && uIsFinite(d))
          {
            ...
            {
              if (isInMM) {
                if (fabs(d - tmp) < maxZError) {
                  tmp += d;
                  ++count;
                }
              }
              else {
                if (fabs(d - tmp) < maxZError * 0.001f) {
                  tmp += d;
                  ++count;
                }
              }
...
pcl::PointXYZ projectDepthTo3D(const cv::Mat &depthImage,
                               float x, float y, float cx, float cy,
                               float fx, float fy, bool smoothing, float maxZError)
{
  pcl::PointXYZ pt;
                              ...
                              float maxDepth, float minDepth, pcl::PointXYZ &point)
{
  ...
  point = projectDepthTo3D(depth, col * 1.0f, row * 1.0f, cx, cy, fx, fy, true, 10.0);
  
  if (isnan(point.x) || isnan(point.y) || isnan(point.z) ||
      point.z < minDepth || point.z > maxDepth)
    return false;
  else
    return true;
}
